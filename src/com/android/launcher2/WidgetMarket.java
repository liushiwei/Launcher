...
public class WidgetMarket extends AppWidgetProvider {
	private static final String TAG="WidgetMarket";
	private static final String PKG = "com.carit.market";
	private static final String CLS = "com.carit.market.CaritMarketActivity";
	private static final String WIDGET_APP_ACTION="com.android.launcher.intent.action.MARKET";
	...
	void updateAppWidget(Context context,RemoteViews remoteViews,int[] appWidgetIds){
		...
		if(appWidgetIds == null){
			appWidgetIds = appWidgetManager.getAppWidgetIds(new ComponentName(
                context, WidgetMarket.class));
		}
		
		if(appWidgetIds == null){
			appWidgetManager.updateAppWidget(new ComponentName(
					context,WidgetMarket.class),remoteViews);
		}else{
