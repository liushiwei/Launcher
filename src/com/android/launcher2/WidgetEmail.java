...
public class WidgetEmail extends AppWidgetProvider {
	private static final String TAG="WidgetEmail";
	private static final String PKG = "com.android.email";
	private static final String CLS = "com.android.email.activity.Welcome";
	private static final String WIDGET_APP_ACTION="com.android.launcher.intent.action.EMAIL";
	...
	void updateAppWidget(Context context,RemoteViews remoteViews,int[] appWidgetIds){
		...
		if(appWidgetIds == null){
			appWidgetIds = appWidgetManager.getAppWidgetIds(new ComponentName(
                context, WidgetEmail.class));
		}
		
		if(appWidgetIds == null){
			appWidgetManager.updateAppWidget(new ComponentName(
					context,WidgetEmail.class),remoteViews);
		}else{
