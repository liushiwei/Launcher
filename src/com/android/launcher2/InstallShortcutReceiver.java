...
public class InstallShortcutReceiver extends BroadcastReceiver {
	...
	private boolean installShortcut(Context context, Intent data, int screen) {
		...
		int screen_tmp = data.getIntExtra(Launcher.EXTRA_SHORTCUT_SCREEN, -1);
		if (screen_tmp < 0) {
			return false;
		} else {
			screen = screen_tmp;
		...
		if (findEmptyCell(context, mCoordinates, screen)) {
			...
			int cellX_tmp = data.getIntExtra(Launcher.EXTRA_SHORTCUT_CELL_X, -1);
			int cellY_tmp = data.getIntExtra(Launcher.EXTRA_SHORTCUT_CELL_Y, -1);
			// cell.cellX = mCoordinates[0];
			// cell.cellY = mCoordinates[1];
			cell.cellX = cellX_tmp;
			cell.cellY = cellY_tmp;
			cell.screen = screen;
	...
	private static boolean findEmptyCell(Context context, int[] xy, int screen) {
		...
		Cursor c = cr.query(LauncherSettings.Favorites.CONTENT_URI, new String[] { LauncherSettings.Favorites.CELLX,
				LauncherSettings.Favorites.CELLY, LauncherSettings.Favorites.SPANX, LauncherSettings.Favorites.SPANY },
				LauncherSettings.Favorites.SCREEN + "=?", new String[] { String.valueOf(screen) }, null);
